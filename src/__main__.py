...
def parsing_args():
    ...
    pars.add_argument("--input", type=str, default="data/input/function_calling_tests.json")
    pars.add_argument("--functions_definition", type=str, default="data/input/functions_definition.json")
    pars.add_argument("--output", type=str, default="data/output/functions_output.json")
    pars.add_argument("--model", type=str, default="Qwen/Qwen3-0.6B")
...
def main():
    ...
    for index, p in enumerate(prompts, 1):
        prompt = p.prompt
        console.print(f"[bold cyan]Processing prompt {index}/{len(prompts)}:[/bold cyan] {prompt}")
        ...
        if parsed.get("name", "none") != "none":
            console.print(f"  -> [bold green]✅ {parsed['name']}({parsed['args']})[/bold green]")
        else:
            console.print("  -> [bold red]❌ [Error] Could not generate function call[/bold red]")
        print() 
    ...
    console.print(f"Total time: [bold yellow]{total_time:.2f}[/bold yellow] seconds\n")
